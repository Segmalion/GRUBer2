...
int WINAPI _tWinMain(HINSTANCE, HINSTANCE, LPTSTR, int)
{
	try
	{
		...
		if (ParamCount() > 0)
		{
			...
			if (silent) {
				// "-silent" обязан сопровождаться путём к выходному файлу. Если это последний
				// аргумент (outputPath пуст) или следующий аргумент сам похож на другой флаг
				// (например, кто-то забыл путь и написал "-silent -cat 2"), путь считаем
				// отсутствующим и выходим с отдельным кодом ошибки, вместо того чтобы молча
				// пытаться сохранить JSON в пустой/неверный путь.
				if (outputPath.IsEmpty() || outputPath[1] == L'-') {
					OutputDebugStringW(L"DeviceLister -silent: не вказано шлях до вихідного JSON-файлу.");
					return 2;
				}
